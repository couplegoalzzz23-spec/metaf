...
BMKG_FORECAST_KOTA = "https://api.bmkg.go.id/publik/prakiraan-cuaca?adm2=14.71"
...
try:
    ...
    st.info("Satellite imagery temporarily unavailable.")

# =====================================
# BMKG FORECAST — CITY LEVEL
# =====================================
st.divider()
st.subheader("🌦️ BMKG Weather Forecast — Kota Pekanbaru")
st.caption("BMKG Public API — Non-ICAO | Situational Awareness")

try:
    r = requests.get(BMKG_FORECAST_KOTA, timeout=10)
    if r.status_code == 200:
        ...
        if lokasi:
            st.caption(f"{lokasi.get('kota','Pekanbaru')} — {lokasi.get('provinsi','Riau')}")

        if prakiraan:
            for f in prakiraan[:8]:
                st.write(
                    f"🕒 {f.get('jamCuaca','-')} | "
                    f"{f.get('cuaca','-')} | "
                    f"🌡️ {f.get('temp','-')}°C | "
                    f"💧 RH {f.get('humidity','-')}% | "
                    f"💨 {f.get('kecepatanAngin','-')} km/h"
                )
        else:
            st.info("BMKG forecast not issued at this time.")
    else:
        st.info("BMKG forecast temporarily unavailable.")
except Exception:
    st.info("BMKG forecast service unreachable.")
