...
# =====================================
st.set_page_config(
    page_title="Tactical Weather Ops — BMKG | QAM METOC WIBB",
    page_icon="✈️",
    layout="wide"
)
...
def fetch_metar_history(hours=24):
    ...
    r.raise_for_status(); return r.text.strip().splitlines()

def fetch_metar_ogimet(hours=24):
    end = datetime.utcnow(); start = end - pd.Timedelta(hours=hours)
    url = "https://www.ogimet.com/display_metars2.php"
    params = {"lang":"en","lugar":"WIBB","tipo":"ALL","ord":"REV","nil":"NO","fmt":"txt",
...
def temp_dew(m): x = re.search(r' (M?\d{2})/(M?\d{2})', m); return f"{x.group(1)} / {x.group(2)} °C" if x else "-"
...
def generate_pdf(lines):
    content="BT\n/F1 10 Tf\n72 800 Td\n"
    for l in lines: safe=l.replace("\\","\\\\").replace("(","\\(").replace(")","\\)"); content+=f"({safe}) Tj\n0 -14 Td\n"
    content+="ET"
...
def flatten_cuaca_entry(entry):
    ...
    for group in entry.get("cuaca",[]):
        for obs in group:
            r=obs.copy()
            r.update({"adm1":lokasi.get("adm1"),"adm2":lokasi.get("adm2"),"provinsi":lokasi.get("provinsi"),
                      "kotkab":lokasi.get("kotkab"),"lon":lokasi.get("lon"),"lat":lokasi.get("lat")})
            try: r["utc_datetime_dt"]=pd.to_datetime(r.get("utc_datetime")); r["local_datetime_dt"]=pd.to_datetime(r.get("local_datetime"))
            except Exception: r["utc_datetime_dt"], r["local_datetime_dt"]=pd.NaT,pd.NaT
            rows.append(r)
...
with st.sidebar:
    ...
    page = st.selectbox("🌐 Navigation", ["Home","QAM METAR","Forecast BMKG","Satellite","Historical"])
...
if page=="Home":
    ...
    metar = fetch_metar()
    st.metric("Surface Wind", wind(metar))
    st.metric("Visibility", visibility(metar))
    ...
    if not raw or len(raw)<2: raw = fetch_metar_ogimet(24); source="OGIMET Archive"
    df_metar = pd.DataFrame([parse_numeric_metar(m) for m in raw if parse_numeric_metar(m)])
    st.caption(f"Data source: {source} | Records: {len(df_metar)}")
    if not df_metar.empty:
        df_metar.sort_values("time", inplace=True)
        fig = make_subplots(rows=5, cols=1, shared_xaxes=True,
                            subplot_titles=["Temp/Dew","Wind","QNH","Visibility","Weather Flags"])
        fig.add_trace(go.Scatter(x=df_metar["time"], y=df_metar["temp"], name="Temp"),1,1)
        fig.add_trace(go.Scatter(x=df_metar["time"], y=df_metar["dew"], name="Dew"),1,1)
        fig.add_trace(go.Scatter(x=df_metar["time"], y=df_metar["wind"], name="Wind"),2,1)
        fig.add_trace(go.Scatter(x=df_metar["time"], y=df_metar["qnh"], name="QNH"),3,1)
        fig.add_trace(go.Scatter(x=df_metar["time"], y=df_metar["vis"], name="Visibility"),4,1)
    ...
    with st.spinner("🛰️ Fetching forecast data..."):
        try: raw = fetch_forecast(adm1)
        except Exception as e: st.error(f"Failed: {e}"); st.stop()
    entries = raw.get("data",[])
    if not entries: st.warning("No forecast data"); st.stop()
    mapping={}; 
    for e in entries: lok=e.get("lokasi",{}); mapping[lok.get("kotkab") or lok.get("adm2") or f"Loc {len(mapping)+1}"]={"entry":e}
    loc_choice = st.selectbox("🎯 Location", options=list(mapping.keys()))
    df = flatten_cuaca_entry(mapping[loc_choice]["entry"])
    df["ws_kt"]=df["ws"]*MS_TO_KT
    if not df.empty: df.sort_values("utc_datetime_dt", inplace=True)
    ...
    st.subheader("📊 Trends")
    c1,c2=st.columns(2)
    with c1: st.plotly_chart(px.line(df,x="local_datetime_dt",y="t",markers=True,color_discrete_sequence=["#a9df52"]),use_container_width=True)
    with c2: st.plotly_chart(px.line(df,x="local_datetime_dt",y="ws_kt",markers=True,color_discrete_sequence=["#00ffbf"]),use_container_width=True)
    
elif page=="Satellite":
    st.title("🛰️ Satellite Himawari-8")
    st.caption("Reference only")
    try:
        img = requests.get(SATELLITE_HIMA_RIAU, timeout=10, headers={"User-Agent":"Mozilla/5.0"}); img.raise_for_status()
        st.image(img.content, use_container_width=True)
    except: st.warning("Satellite imagery unavailable")

elif page=="Historical":
    st.title("📥 Historical METAR")
    raw = fetch_metar_history(24)
    if not raw or len(raw)<2: raw = fetch_metar_ogimet(24)
    df_metar=pd.DataFrame([parse_numeric_metar(m) for m in raw if parse_numeric_metar(m)])
    st.dataframe(df_metar)
    st.download_button("⬇️ Download CSV", df_metar.to_csv(index=False), "WIBB_METAR_24H.csv")
    st.download_button("⬇️ Download JSON", df_metar.to_json(orient="records"), "WIBB_METAR_24H.json")
...
QAM METOC WIBB — Lanud Roesmin Nurjadin<br>
Designed with Military Precision | Streamlit + Plotly
</div>
""", unsafe_allow_html=True)
